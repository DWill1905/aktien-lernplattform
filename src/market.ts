...
const marketReturnsCache: number[] = [];
const marketRng = mulberry32(MARKET_SEED);
let regimeDaysLeft = 0;
let regimeDrift = 0;

function marketReturns(days: number): number[] {
...
export interface Quote {
  /** Mittelkurs – Basis für Depotbewertung und Charts. */
  mid: number;
  /** Geldkurs (Bid): Preis, den Verkäufer erhalten. */
  bid: number;
  /** Briefkurs (Ask): Preis, den Käufer zahlen. */
  ask: number;
}
 ...
 */
export function quote(stock: Stock, day: number): Quote {
