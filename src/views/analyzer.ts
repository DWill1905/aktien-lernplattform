...
import { Candle, generateCandles, sma, rsi } from "../indicators.js";
...
interface ActiveCall extends Decision {
  side: Side;
}
...
function drawCandles(canvas: HTMLCanvasElement, candles: Candle[], smaValues: (number | null)[] | null, lines: number[]): Viewport {
  ...
  const xAt = (i: number): number => padX + slot * i + slot / 2;
...
function drawRsi(canvas: HTMLCanvasElement, values: (number | null)[]): void {
  const ctx = canvas.getContext("2d")!;
  const dpr = window.devicePixelRatio || 1;
  const width = canvas.clientWidth || 600;
  ...
  const xAt = (i: number): number => padX + slot * i + slot / 2;
  const yAt = (v: number): number => padY + (1 - v / 100) * plotH;
...
function resultCardChildren(...nodes: (HTMLElement | null)[]): HTMLElement[] {
  const present = nodes.filter((n): n is HTMLElement => n !== null);
  return present.length ? present : [el("p", { class: "muted" }, ["Noch keine Einschätzung abgegeben."])];
}

export function renderAnalyzer(): HTMLElement {
  const container = el("div", {});

  let selectedStockId = STOCKS[0].id;
  let fullCandles: Candle[] = [];
  ...
  let openDecision: Decision | null = null;
  ...
  function advance(): void {
    if (openDecision || revealCount >= fullCandles.length) return;
    revealCount++;
  ...
  function renderCanvas(): void {
    ...
    if (showRsi) drawRsi(rsiCanvas, rsi(closes, RSI_PERIOD));
  }

  function updateProgressLabel(): void {
  ...
  function build(): HTMLElement {
    ...
    mainCanvas = el("canvas", { class: "chart analyzer-chart" }) as HTMLCanvasElement;
    mainCanvas.addEventListener("click", (e) => {
      if (!lineDrawMode || !viewport) return;
    ...
    rsiCanvas = el("canvas", { class: "chart analyzer-rsi", style: showRsi ? "" : "display:none" }) as HTMLCanvasElement;
    ...
    const smaCheckbox = el("input", { type: "checkbox", checked: showSma }) as HTMLInputElement;
    smaCheckbox.addEventListener("change", () => {
      showSma = smaCheckbox.checked;
      renderCanvas();
    });
    const rsiCheckbox = el("input", { type: "checkbox", checked: showRsi }) as HTMLInputElement;
    ...
    const lineModeBtn = el(
      "button",
      { class: `btn secondary${lineDrawMode ? " active" : ""}`, "aria-pressed": String(lineDrawMode) },
      ["📏 Linie zeichnen"]
    ...
    const playBtn = el("button", { class: "btn" }, [playing ? "⏸ Pause" : "▶ Abspielen"]) as HTMLButtonElement;
    if (openDecision || finished) playBtn.setAttribute("disabled", "true");
    playBtn.addEventListener("click", togglePlay);

    const stepBtn = el("button", { class: "btn secondary" }, ["⏭ Einzelschritt"]) as HTMLButtonElement;
    if (openDecision || finished || playing) stepBtn.setAttribute("disabled", "true");
