...
export function renderPortfolio(): HTMLElement {
  ...
  function build(): HTMLElement {
    ...
    const priceInfo = (stock: (typeof STOCKS)[number]): { price: number; change: number } => {
    ...
    const starButton = (stockId: string, watched: boolean): HTMLButtonElement => {
      const btn = el("button", {
      ...
      }, [symbol("star", watched)]) as HTMLButtonElement;
      btn.addEventListener("click", () => {
        ...
        : el("table", {}, [
            ...
            el(
              ...
              watchedStocks.map((stock) => {
                const { price, change } = priceInfo(stock);
                const selectBtn = el("button", { class: "btn secondary btn-inline" }, ["Auswählen"]) as HTMLButtonElement;
    ...
    const takeProfitInput = el("input", { type: "number", min: "0", step: "0.01", placeholder: "z. B. 55,00", id: "trade-takeprofit" }) as HTMLInputElement;
    ...
    const updatePreview = (): void => {
      ...
      preview.replaceChildren(
        el("span", {}, [`Geld/Brief ${q.bid.toFixed(2)} / ${q.ask.toFixed(2)} €`]),
        el("span", {}, [`Kaufvolumen ${formatCurrency(buyVolume)}`]),
        el("span", {}, [`Gebühr ${formatCurrency(fee)}`]),
        el("span", { class: buyVolume + fee > state.cash ? "neg" : "" }, [`Kauf gesamt ${formatCurrency(buyVolume + fee)}`]),
    ...
    const submit = (side: "buy" | "sell"): void => {
      const shares = sharesInput.valueAsNumber;
      const kind = orderKindSelect.value;
      const tif = tifSelect.value as "gtc" | "day";
      const result =
        kind === "market"
          ? side === "buy"
          ...
          : kind === "trailing"
            ...
            : kind === "bracket"
              ...
              : placeOrder(stockSelect.value, side, kind as "limit" | "stop", shares, triggerInput.valueAsNumber, tif);
    ...
    const riskPctInput = el("input", { type: "number", min: "0.1", step: "0.1", value: "1" }) as HTMLInputElement;
    const entryInput = el("input", { type: "number", min: "0", step: "0.01", value: currentPrice(stockById(selectedStockId)!, state.day).toFixed(2) }) as HTMLInputElement;
    const stopInput = el("input", { type: "number", min: "0", step: "0.01", placeholder: "Stop-Kurs" }) as HTMLInputElement;
    ...
    const recompute = (): void => {
      ...
      if (suggestedShares > 0) applyCalcBtn.removeAttribute("disabled");
      else applyCalcBtn.setAttribute("disabled", "true");
    };
    ...
    const scenarioButtons = CRASH_SCENARIOS.map((scenario) =>
      makeButton(`${scenario.name} (${scenario.period})`, "secondary", () => {
    ...
    const crashReport = crashResult
      ? el("div", { class: "card crash-report" }, [
          ...
          crashResult.positionImpacts.length === 0
            ...
            : el("table", {}, [
                ...
                el(
                  "tbody",
                  {},
                  crashResult.positionImpacts.map((p) =>
                    el("tr", {}, [
                      el("td", {}, [p.name]),
                      el("td", {}, [p.sector]),
          ...
          el(
            "ul",
            { class: "crash-recommendations" },
            crashResult.recommendations.map((r) => el("li", {}, [r]))
          ),
        ])
      : null;
