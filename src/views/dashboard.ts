...
export function renderDashboard(): HTMLElement {
  ...
  const achievementIcons = ACHIEVEMENTS.map((a) =>
    el("span", { class: `achievement-icon${unlockedIds.has(a.id) ? " unlocked" : ""}`, title: unlockedIds.has(a.id) ? `${a.title}: ${a.description}` : "Noch nicht freigeschaltet" }, [symbol(a.icon, unlockedIds.has(a.id))])
  );
  ...
  const resetProgressBtn = el("button", { class: "btn-ghost-danger" }, ["Lernfortschritt zurücksetzen"]) as HTMLButtonElement;
  resetProgressBtn.addEventListener("click", () => {
    confirmDestructiveAction({
      title: "Lernfortschritt zurücksetzen?",
      message:
        "Alle als gelesen markierten Lektionen, Quiz-Ergebnisse, Erfolge und Wiederholungs-Karteikarten gehen unwiderruflich verloren.",
