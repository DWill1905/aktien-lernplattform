...
/** Spaltendefinition: sortierbarer Zahlenwert + Anzeige. Null sortiert ans Ende. */
interface Column {
  key: string;
  label: string;
  sortValue: (row: ScreenerRow) => number | null;
...
function ratingChip(label: string, value: string, assessment: MetricAssessment): HTMLElement {
...
export function renderScreener(): HTMLElement {
  ...
  const rows: ScreenerRow[] = STOCKS.map((stock) => {
  ...
  function build(): HTMLElement {
    ...
    if (column) {
      visible.sort((a, b) => {
        ...
        if (va === null) return 1; // nicht berechenbare Werte immer ans Ende
        if (vb === null) return -1;
    ...
    const filterChips = FILTERS.map((filter, i) => {
      ...
      chip.addEventListener("click", () => {
        if (active) activeFilters.delete(i);
        else activeFilters.add(i);
    ...
    if (selected) {
      ...
      detailCard = el("div", { class: "card card-outlined" }, [
        ...
        el("div", { class: "rating-grid" }, [
          ...
          ratingChip(
            "Ausschüttungsquote",
            m.payoutRatio !== null && m.payoutRatio > 0 ? formatPercent(m.payoutRatio).replace("+", "") : "–",
            assessPayout(m.payoutRatio)
          ),
        ]),
        el("p", { class: "muted" }, [
