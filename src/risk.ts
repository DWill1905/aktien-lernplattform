...
function diversificationLabel(hhi: number): string {
  if (hhi < 1500) return "gut diversifiziert";
...
function syntheticValueSeries(state: PortfolioState): number[] {
  ...
  for (let d = 0; d <= days; d++) {
    ...
    for (const [stockId, position] of Object.entries(state.positions)) {
      ...
      if (stock) value += position.shares * currentPrice(stock, d);
    }
    series.push(value);
  }
  return series;
...
export function computeHealthCheck(state: PortfolioState): HealthCheckResult {
  ...
  for (const p of positionValues) bySector.set(p.sector, (bySector.get(p.sector) ?? 0) + p.value);

  const sectorWeights: SectorWeight[] = [...bySector.entries()]
