...
export const CRASH_SCENARIOS = [
    {
        ...
        name: "Dotcom-Bubble",
        period: "2000–2002",
        description: "Platzen der Internet-Blase: Technologieaktien verloren binnen zwei Jahren einen Großteil ihres Wertes, viele frühe Internet-Firmen verschwanden ganz vom Markt.",
        declinePct: 0.49,
    ...
    },
    {
        ...
        period: "2007–2009",
        description: "Ausgelöst durch die US-Immobilienkrise und den Zusammenbruch von Lehman Brothers: Aktienmärkte weltweit brachen massiv ein, zyklische Branchen traf es am härtesten.",
        declinePct: 0.57,
    ...
    },
    {
        ...
        description: "Weltweite Lockdowns wegen der COVID-19-Pandemie lösten den schnellsten Börsencrash der Geschichte aus – gefolgt von einer ebenso schnellen Erholung dank massiver Notenbank-Stützung.",
        declinePct: 0.34,
    ...
    },
    {
        ...
        period: "2022",
        description: "Notenbanken erhöhten die Leitzinsen so schnell wie seit Jahrzehnten nicht mehr, um die hohe Inflation zu bekämpfen – besonders wachstums- und zinssensitive Aktien litten überdurchschnittlich.",
        declinePct: 0.25,
...
function buildRecommendations(cash, valueBefore, impacts, troughLossPct) {
    if (impacts.length === 0) {
    ...
    for (const p of impacts)
        bySector.set(p.sector, (bySector.get(p.sector) ?? 0) + p.valueBefore);
    const topSector = [...bySector.entries()].sort((a, b) => b[1] - a[1])[0];
    if (topSector && totalInvested > 0 && topSector[1] / totalInvested >= 0.5) {
        recs.push(`Dein hoher Anteil im Sektor „${topSector[0]}" hat den Verlust in diesem Szenario verstärkt – breitere Streuung über Branchen hätte ihn abgefedert.`);
    }
 ...
 */
export function simulateCrash(state, scenario) {
    const holdings = Object.entries(state.positions).filter(([, pos]) => pos.shares > 0);
    const positionImpacts = holdings.map(([stockId, pos]) => {
