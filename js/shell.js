...
export function symbol(name, filled = false) {
    return el("span", { class: `msym${filled ? " filled" : ""}`, "aria-hidden": "true" }, [name]);
}
function pageInfo(hash) {
    ...
    if (key)
        return { title: STATIC_TITLES[key], back: "#/" };
...
function syncMore() {
    ...
    document.querySelectorAll(".more-trigger").forEach((b) => b.setAttribute("aria-expanded", String(moreOpen)));
}
function buildMoreSheet() {
    ...
    sheet.replaceChildren(...MORE_ITEMS.map((item) => {
...
// --- Navigation ------------------------------------------------------------
function navLink(item, hash, variant) {
...
function moreTrigger(variant) {
    const btn = el("button", { class: `${variant}-item more-trigger`, type: "button", "aria-expanded": "false", "aria-haspopup": "menu" }, [el("span", { class: "nav-icon" }, [symbol("more_horiz")]), el("span", { class: "nav-label" }, ["Mehr"])]);
...
function buildRail(hash) {
    const rail = document.getElementById("rail");
    if (!rail)
        return;
    rail.replaceChildren(el("a", { class: "rail-brand", href: "#/", "aria-label": "Börsenschule – Startseite" }, [
        el("span", { class: "logo-bar" }, []),
        el("span", { class: "logo-bar accent" }, []),
        el("span", { class: "logo-bar tall" }, []),
    ]), el("div", { class: "rail-items" }, NAV_ITEMS.map((i) => navLink(i, hash, "rail"))), moreTrigger("rail"));
}
function buildBottomNav(hash) {
    const nav = document.getElementById("bottomnav");
    if (!nav)
        return;
    nav.replaceChildren(...NAV_ITEMS.map((i) => navLink(i, hash, "bottom")), moreTrigger("bottom"));
}
function buildAppBar(hash) {
    ...
    bar.replaceChildren(left, el("div", { class: "appbar-spacer" }, []), el("div", { class: "appbar-chips" }, [
        gami.streak > 0
